...
struct __split_buffer {
 ...
 private:
  void move_assign_alloc_(__split_buffer& x, true_type) noexcept(std::is_nothrow_move_assignable<allocator_remove_reference_type_>::value) {
...
// postcondition: size() == size() + n
template <class T, class Allocator>
void __split_buffer<T, Allocator>::copy_construct_at_end_(size_type n, const value_type& value) {
  do {
...
template <class ForwardIterator>
typename enable_if<__is_forward_iterator<ForwardIterator>::value, void>::type
__split_buffer<T, Allocator>::copy_construct_at_end_(ForwardIterator first, ForwardIterator last) {
  do {
    // copy constructor may throw
    alloc_traits_::construct(this->alloc_, __to_raw_pointer(this->end_), *first);
    // if construct success, add end_
    ++this->end_;
  } while(++first != last);
}
...
template <class T, class Allocator>
void __split_buffer<T, Allocator>::destruct_at_begin_(pointer new_begin) noexcept {
...
template <class T, class Allocator>
__split_buffer<T, Allocator>::__split_buffer(__split_buffer&& x)
...
__split_buffer<T, Allocator>::__split_buffer(__split_buffer&& x, allocator_remove_reference_type_& alloc)
  : storage_(nullptr),
    ...
    if(x.alloc == alloc_) {
      ...
      copy_construct_at_end_(std::move_iterator<iterator>(x.begin_), std::move_iterator<iterator>(x.end_));
    }
}
